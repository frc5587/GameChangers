...
public class Limelightold extends SubsystemBase {
  ...
  /**
   * Calculates the optimum angle to set the arm, in order to use the least amount
   * of power shooting the powercells. This means that the powercell must enter
   * the power port at the top of its arc.
   * 
   * @param distanceMeters the horizontal distance between the shooter and target
   *                       - METERS
  ...
  /**
   * Calculates the speed that the shooter should spin at to get the power cell in
   * the power port using the lowest speed possible
   * 
   * @param currentArmAngleRadians arm angle - RADIANS
   * @param t                      the target
